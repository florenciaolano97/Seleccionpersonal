...
class AlbaAIEngine:
    ...
    def _call(self, messages: list[dict], temperature: float = 0.2, max_tokens: int = 2000, json_mode: bool = True) -> dict | str:
        if not self.client:
...
"""
        ...
        result.setdefault("config", {"preset_name": "IA Semántica", "groups": [], "thresholds": {"advance": 75, "review": 50, "minimum_required_coverage": 50}})
...
"""
    ...
    def generate_final_report(self, candidate_name: str, job_title: str, evaluations: list[dict]) -> dict:
...
# =========================================================
# AVATAR CONVERSACIONAL INTELIGENTE
# =========================================================

class AlbaConversationalAvatar:
    """Agente de entrevista conversacional en tiempo real usando D-ID Streaming + LLM."""

    def __init__(self, company_id: int, interview_id: int, api_key: str | None = None, model: str = "gpt-4o-mini"):
        self.company_id = company_id
        self.interview_id = interview_id
        self.ai = AlbaAIEngine(api_key=api_key, model=model)
        self.settings = get_avatar_settings(company_id)
    ...
    def _load_questions(self):
        interview = fetch_one("SELECT * FROM interviews WHERE id = ?", (self.interview_id,))
        if interview:
            self.questions = get_template_questions(interview["template_id"], active_only=True)

    def start_session(self) -> str:
        """Genera el mensaje de bienvenida y la primera pregunta."""
        interview = fetch_one("""
            SELECT interviews.*, users.full_name AS candidate_name, jobs.title AS job_title
            FROM interviews
        ...
        candidate_name = interview.get("candidate_name", "Candidato/a") if interview else "Candidato/a"
        job_title = interview.get("job_title", "la posición") if interview else "la posición"

        welcome = (
            f"Hola {candidate_name}, soy ALBA, tu entrevistadora virtual para {job_title}. "
            f"{self.settings.get('welcome_text', 'Voy a hacerte algunas preguntas. Tomate tu tiempo para responder.')}"
        )
    ...
    def process_answer(self, answer_text: str, question: dict) -> dict:
        ...
        # Decidir si hacer follow-up dinámico
        follow_up = None
        if evaluation.get("score", 0) is not None and float(evaluation.get("score", 0)) < 3.0:
    ...
    def _generate_follow_up(self, question: dict, answer_text: str) -> str:
        system = "Sos ALBA, una entrevistadora amable pero exigente. Pedí una aclaración o profundización."
        prompt = f"""La respuesta del candidato a la pregunta "{question.get('question_text')}" fue insuficiente.

RESPUESTA: {answer_text[:1000]}

Generá UNA pregunta de follow-up breve y amable para que profundice en la situación, acción o resultado. Máximo 20 palabras."""
        try:
            return str(self.ai._call([
                {"role": "system", "content": system},
                {"role": "user", "content": prompt},
            ], json_mode=False))
        except Exception:
    ...
    def generate_closing(self) -> str:
        return "Muchas gracias por tu tiempo. La entrevista ha finalizado. RR. HH. revisará tus respuestas y se contactará contigo."

    def save_transcript(self):
        """Guarda el transcript en la sesión de entrevista."""
        transcript = "\n\n".join([f"{'ALBA' if m['role'] == 'assistant' else 'Candidato/a'}: {m['content']}" for m in self.history])
        existing = fetch_one("SELECT id FROM interview_sessions WHERE interview_id = ?", (self.interview_id,))
        if existing:
            execute("""
                UPDATE interview_sessions
                SET transcript_text = ?, updated_at = ?
                WHERE interview_id = ?
            """, (transcript, now_iso(), self.interview_id))
        else:
            execute("""
                INSERT INTO interview_sessions (interview_id, transcript_text, created_at, updated_at)
                VALUES (?, ?, ?, ?)
            """, (self.interview_id, transcript, now_iso(), now_iso()))
...
def render_company_settings(user: dict, company: dict) -> None:
    ...
    st.markdown("---")
    st.subheader("🧠 Configuración de Inteligencia Artificial")
    cfg = get_openai_config()
    c1, c2 = st.columns(2)
    c1.metric("Motor IA", "Activo" if cfg["enabled"] else "Inactivo")
    c2.metric("Modelo", cfg["model"] if cfg["enabled"] else "N/A")

    if not cfg["enabled"]:
        st.warning(
            "El análisis semántico con IA no está activo. "
            "Agregá OPENAI_API_KEY en Streamlit Secrets para habilitarlo."
        )
        with st.expander("Ejemplo de configuración"):
            st.code(
                'OPENAI_API_KEY = "sk-..."\n'
        ...
        st.success("El motor de IA semántica está operativo.")
        st.caption(AI_ENGINE_NOTICE)
...
def parse_cv_text(
    ...
    # --- Análisis semántico con IA (fallback al parser clásico) ---
    cfg = get_openai_config()
    if cfg["enabled"]:
        try:
            engine = AlbaAIEngine(api_key=cfg["api_key"], model=cfg["model"])
            ia_result = engine.analyze_cv(text)
            if ia_result.get("full_name") and ia_result.get("email"):
                return ia_result
        except Exception:
            pass
    # ---------------------------------------------------------------
...
def calculate_match_score(
    ...
    # --- Matching semántico con IA (fallback al algoritmo clásico) ---
    cfg = get_openai_config()
    if cfg["enabled"]:
        try:
            engine = AlbaAIEngine(api_key=cfg["api_key"], model=cfg["model"])
            return engine.calculate_semantic_match(job, cv_text)
        except Exception:
            pass
    # -----------------------------------------------------------------
...
def evaluate_interview_answer(
    ...
    # --- Evaluación semántica con IA (fallback al algoritmo clásico) ---
    cfg = get_openai_config()
    if cfg["enabled"]:
        try:
            engine = AlbaAIEngine(api_key=cfg["api_key"], model=cfg["model"])
            return engine.evaluate_response(question, answer_text)
        except Exception:
            pass
    # -------------------------------------------------------------------
...
def render_conversational_interview(
    user: dict,
    candidate: dict,
    interview: dict,
    avatar_settings: dict,
) -> None:
    """Renderiza la experiencia de entrevista conversacional con ALBA."""
    st.markdown("### 🎙️ Entrevista conversacional con ALBA")
    st.info(
        "ALBA te hará preguntas de voz y podés responder por escrito o hablando. "
        "La entrevista se adapta a tus respuestas."
    )

    session_key = f"conv_avatar_{interview['id']}"
    if session_key not in st.session_state:
        st.session_state[session_key] = AlbaConversationalAvatar(
            company_id=interview["company_id"],
            interview_id=interview["id"],
        )
    agent = st.session_state[session_key]

    # Inicializar conversación
    if not agent.history:
        welcome = agent.start_session()
        agent.history.append({"role": "assistant", "content": welcome})

    # Mostrar historial
    for msg in agent.history:
        if msg["role"] == "assistant":
            with st.chat_message("ALBA", avatar="🤖"):
                st.write(msg["content"])
        else:
            with st.chat_message("Candidato/a", avatar="👤"):
                st.write(msg["content"])

    # Estado actual
    current_q = agent.get_next_question()
    if current_q and not any(m.get("role") == "user" for m in agent.history[-2:]):
        with st.chat_message("ALBA", avatar="🤖"):
            st.write(current_q["question_text"])
            agent.history.append({"role": "assistant", "content": current_q["question_text"]})

    # Input del candidato
    answer = st.chat_input("Escribí tu respuesta aquí...")
    if answer and current_q:
        with st.chat_message("Candidato/a", avatar="👤"):
            st.write(answer)

        result = agent.process_answer(answer, current_q)

        # Guardar respuesta en BD
        existing = fetch_one("""
            SELECT id FROM interview_responses
            WHERE interview_id = ? AND question_id = ?
        """, (interview["id"], current_q["id"]))

        if existing:
            execute("""
                UPDATE interview_responses
                SET answer_text = ?, submitted_at = ?, updated_at = ?
                WHERE interview_id = ? AND question_id = ?
            """, (answer, now_iso(), now_iso(), interview["id"], current_q["id"]))
        else:
            execute("""
                INSERT INTO interview_responses (interview_id, question_id, answer_text, submitted_at, updated_at)
                VALUES (?, ?, ?, ?, ?)
            """, (interview["id"], current_q["id"], answer, now_iso(), now_iso()))

        # Mostrar evaluación y follow-up
        with st.chat_message("ALBA", avatar="🤖"):
            eval_data = result["evaluation"]
            score = eval_data.get("score")
            if score is not None:
                st.caption(f"Puntaje asistido: {score}/{eval_data.get('maximum', 5)}")

            if result.get("follow_up"):
                st.write(result["follow_up"])
                agent.history.append({"role": "assistant", "content": result["follow_up"]})
            elif result.get("finished"):
                closing = agent.generate_closing()
                st.success(closing)
                agent.history.append({"role": "assistant", "content": closing})
                agent.save_transcript()
                execute("""
                    UPDATE interviews
                    SET status = 'COMPLETADA', completed_at = ?, updated_at = ?
                    WHERE id = ?
                """, (now_iso(), now_iso(), interview["id"]))
                save_automatic_interview_evaluation(interview["id"])
                st.balloons()
            else:
                next_q = agent.get_next_question()
                if next_q:
                    st.write(next_q["question_text"])
                    agent.history.append({"role": "assistant", "content": next_q["question_text"]})
                else:
                    closing = agent.generate_closing()
                    st.success(closing)
                    agent.history.append({"role": "assistant", "content": closing})
    ...
    if st.button("Finalizar entrevista y guardar", key=f"finish_conv_{interview['id']}"):
        agent.save_transcript()
        execute("""
            UPDATE interviews
            SET status = 'COMPLETADA', completed_at = ?, updated_at = ?
        ...
        """, (now_iso(), now_iso(), interview["id"]))
        save_automatic_interview_evaluation(interview["id"])
        st.success("Entrevista finalizada. RR. HH. revisará tus respuestas.")
        st.balloons()
        st.rerun()
