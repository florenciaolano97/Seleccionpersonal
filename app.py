...
def normalize_did_api_key(api_key: str) -> str:
    """
    D-ID usa Authorization: Basic <KEY>.
    Si el usuario pega la key con la palabra Basic incluida, la limpiamos para evitar:
    Authorization: Basic Basic xxxx
    """
...
def did_headers(api_key: str):
    key = normalize_did_api_key(api_key)
    return {
...
def create_did_talk(text: str, source_url: str, api_key: str, voice_id: str, presenter_name: str = "Alba"):
    ...
    if not is_public_image_url(source_url):
        raise ValueError("D_ID_SOURCE_URL debe ser una URL pública HTTPS directa a una imagen .jpg, .png o .webp.")
...
def poll_did_talk(talk_id: str, api_key: str, max_wait_seconds: int = 120):
...
with st.sidebar:
    ...
    st.caption("Usa el endpoint correcto de D-ID: POST /talks. No uses /translations, porque eso es para traducir videos ya existentes.")
    if did_source_url and not is_public_image_url(did_source_url):
        st.error("D_ID_SOURCE_URL debe ser una URL pública HTTPS directa a una imagen .jpg, .png o .webp. Ejemplo: https://.../alba.png")
    if did_api_key and did_source_url and is_public_image_url(did_source_url):
        st.success("D-ID configurado: la app podrá generar videos del avatar hablando.")
    else:
        st.info("Cuando configures D_ID_API_KEY + D_ID_SOURCE_URL, el botón de generar video quedará habilitado.")
...
with tab2:
    ...
    if q_idx >= total_q:
        ...
        with left:
            ...
            video_key = f"{role_name}_{q_idx}_{voice_id}_{hash(did_source_url)}"
            ...
            if st.button("🎥 Generar avatar hablando esta pregunta", type="primary", use_container_width=True, disabled=not bool(did_api_key and did_source_url and is_public_image_url(did_source_url))):
                try:
