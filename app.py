...
def transcribe_audio_bytes(audio_bytes: bytes, openai_api_key: str, language: str = "es") -> tuple[str | None, str | None]:
    """Transcribe la respuesta oral del candidato usando OpenAI Whisper/audio transcription."""
    if not audio_bytes:
        return None, "No se recibió audio."
    if not openai_api_key or OpenAI is None:
        return None, "Para transcribir audio necesitás configurar OPENAI_API_KEY."
    try:
        client = OpenAI(api_key=openai_api_key)
        ...
        return getattr(transcription, "text", "").strip(), None
    except Exception as e:
        return None, str(e)
    finally:
        try:
...
def generate_adaptive_next_question(
    ...
    base_next_question: str | None,
    openai_api_key: str,
    model: str,
) -> tuple[str | None, str | None]:
    """Genera una repregunta o siguiente pregunta profesional según la respuesta del candidato."""
    if not openai_api_key or OpenAI is None:
        return base_next_question, "No hay OPENAI_API_KEY. Se usa la pregunta estructurada siguiente."
    try:
        profile = ROLE_PROFILES[role_name]
        client = OpenAI(api_key=openai_api_key)
        history = transcript_text(transcript)
        prompt = f"""
Sos Alba, entrevistadora virtual de RR.HH. para una empresa autopartista plástica.

Puesto: {role_name}
Descripción: {profile['descripcion']}
Competencias: {list(profile['competencies'].keys())}

Historial de entrevista:
{history}

Tu tarea:
- Generar UNA sola próxima intervención breve, natural y profesional.
- Si la última respuesta fue vaga, pedí un ejemplo concreto o profundizá.
- Si la respuesta fue suficiente, podés avanzar hacia esta pregunta estructurada: {base_next_question or 'cierre de entrevista'}.
- No preguntes datos sensibles: edad, hijos, salud, religión, política, nacionalidad, estado civil, domicilio exacto, embarazo, sindicato, discapacidad.
- No hagas más de una pregunta a la vez.
- Redactá en español argentino, tono cálido y formal.
- Devolvé SOLO el texto que Alba tiene que decir, sin comillas ni formato.
"""
        response = client.responses.create(model=model, input=prompt, temperature=0.35)
        text = (getattr(response, "output_text", "") or "").strip()
        if not text:
            return base_next_question, "OpenAI no devolvió texto. Se usa la pregunta estructurada siguiente."
        return text[:700], None
    except Exception as e:
        return base_next_question, str(e)


def get_audio_bytes_from_recorder(audio_obj) -> tuple[bytes | None, str | None]:
    """Normaliza la salida del componente streamlit-mic-recorder."""
    if not audio_obj:
        return None, None
    if isinstance(audio_obj, dict):
        if audio_obj.get("bytes"):
            return audio_obj.get("bytes"), str(audio_obj.get("id", ""))
        if audio_obj.get("audio_bytes"):
            return audio_obj.get("audio_bytes"), str(audio_obj.get("id", ""))
    if isinstance(audio_obj, (bytes, bytearray)):
        return bytes(audio_obj), "raw"
    return None, None
...
with st.sidebar:
    ...
    adaptive_dialogue = st.toggle("Diálogo adaptativo con IA", value=True, help="Después de cada respuesta, Alba puede repreguntar o adaptar la próxima pregunta.")
    max_questions = st.slider("Máximo de preguntas por entrevista", min_value=4, max_value=12, value=8)
    st.divider()
...
st.caption("Streamlit + D-ID + micrófono: Alba habla, escucha/transcribe respuestas y adapta la próxima pregunta.")
...
# Inicializa o reinicia el guion cuando cambia el puesto.
if st.session_state.session_role != role_name or not st.session_state.session_questions:
    st.session_state.session_role = role_name
    st.session_state.session_questions = list(profile["questions"])
    st.session_state.question_index = 0
    st.session_state.transcript = []
    st.session_state.last_result = None

questions = st.session_state.session_questions
q_idx = min(st.session_state.question_index, len(questions) - 1)
...
col_audio, col_text = st.columns([0.85, 1.15])
with col_audio:
    st.markdown("**Opción A — responder hablando**")
    if mic_recorder is None:
        st.warning("Falta instalar `streamlit-mic-recorder`. Ejecutá: pip install streamlit-mic-recorder")
    else:
        audio_obj = mic_recorder(
            ...
            key=f"mic_{role_name}_{q_idx}",
        )
        audio_bytes, audio_id = get_audio_bytes_from_recorder(audio_obj)
        if audio_bytes:
            st.audio(audio_bytes, format="audio/wav")
            if audio_id != st.session_state.last_audio_id:
                st.session_state.last_audio_id = audio_id
            if st.button("📝 Transcribir audio", type="primary", use_container_width=True):
                with st.spinner("Transcribiendo respuesta del candidato..."):
                    text, err = transcribe_audio_bytes(audio_bytes, st.session_state.openai_api_key_ui)
                if err:
                    st.error(err)
                else:
                    st.session_state.pending_transcription = text or ""
                    st.success("Audio transcripto. Revisá el texto antes de guardar.")
                    st.rerun()
    st.caption("Para transcribir audio necesitás OPENAI_API_KEY. La entrevista puede seguir por texto si no querés usar micrófono.")

with col_text:
    st.markdown("**Opción B — revisar/escribir respuesta**")
    answer = st.text_area(
        "Respuesta del candidato",
        value=st.session_state.pending_transcription,
        height=150,
        placeholder="La transcripción aparecerá acá, o podés escribir manualmente la respuesta...",
    )
...
with col_b1:
    if st.button("Guardar respuesta y continuar diálogo", type="primary", use_container_width=True):
        if not answer.strip():
            st.error("Primero grabá/transcribí o escribí la respuesta del candidato.")
        else:
            st.session_state.transcript.append({"question": current_question, "answer": answer.strip()})
            st.session_state.pending_transcription = ""

            # Decide próxima pregunta: adaptativa con IA o estructurada.
            next_index = st.session_state.question_index + 1
            base_next = questions[next_index] if next_index < len(questions) else None

            if len(st.session_state.transcript) >= max_questions or base_next is None:
                st.success("Entrevista finalizada. Ya podés evaluar el perfil.")
            else:
                if adaptive_dialogue:
                    with st.spinner("Alba está pensando la próxima pregunta..."):
                        next_question, err = generate_adaptive_next_question(
                            role_name=role_name,
                            transcript=st.session_state.transcript,
                            base_next_question=base_next,
                            openai_api_key=st.session_state.openai_api_key_ui,
                            model=model_name,
                        )
                    if err:
                        st.info(f"Se usa la pregunta estructurada siguiente. Detalle: {err}")
                    # Reemplaza la próxima pregunta por la versión adaptativa, manteniendo el flujo.
                    if next_question:
                        if next_index < len(st.session_state.session_questions):
                            st.session_state.session_questions[next_index] = next_question
                        else:
                            st.session_state.session_questions.append(next_question)
...
with col_b3:
    if st.button("Reiniciar entrevista", use_container_width=True):
        st.session_state.transcript = []
        st.session_state.question_index = 0
        st.session_state.session_questions = list(ROLE_PROFILES[role_name]["questions"])
        st.session_state.last_result = None
        st.session_state.pending_transcription = ""
        st.rerun()
